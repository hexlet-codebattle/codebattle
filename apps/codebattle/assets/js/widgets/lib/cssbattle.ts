...
const getMatchPoints = (stats: number, width: number, height: number) =>
  1 - stats / (width * height);

const getMatchPercentageText = (match: number) => `${(match * 100).toFixed(2)}%`;

interface BattlePictureMatch {
  match: number;
...
interface MatchBattlePicturesResult {
  result: BattlePictureMatch[];
  targetCanvas: HTMLCanvasElement;
}

export const matchBattlePictures = (
  firstImgDataUrl: string,
  secondImgDataUrl: string,
  targetImgDataUrl: string,
  width: number,
  height: number,
): MatchBattlePicturesResult => {
