...
interface LeaderboardItem {
  ...
  eventPlayersCount?: number | null;
  clansPlayersCount?: number | null;
}

const getCustomEventTrClassNamePersonal = (type: string, muted: boolean, isUser: boolean) =>
...
const tableDataCellClassNamePersonal = (hideSeparator?: boolean) =>
...
const getCustomEventTrClassName = (item: LeaderboardItem, selectedId?: number | string) =>
  cn(
...
interface PersonalEventTableProps {
  currentUserId?: number | string;
  currentUserClanId?: number | string;
...
function PersonalEventTable({
  ...
  items,
  type,
}: PersonalEventTableProps) {
  const groupedItems = Object.values(
    groupBy(items, (item) => item.clanRank),
  ) as LeaderboardItem[][];
            ...
            >
              ...
              <td className={tableDataCellClassNamePersonal()}>
                {users
                  ...
                  .reduce((acc, user) => acc + (user.totalScore ?? 0), 0) || 0}
              </td>
              <td className={tableDataCellClassNamePersonal()}>
                {users
                  .slice(0, maxTopPlayersCount)
                  .reduce((acc, user) => acc + (user.winsCount ?? 0), 0) || 0}
              </td>
              <td className={tableDataCellClassNamePersonal()}>
                {users
                  .slice(0, maxTopPlayersCount)
                  .reduce((acc, user) => acc + (user.totalDurationSec ?? 0), 0) || 0}
              </td>
            </tr>
...
interface EventRatingPanelProps {
...
function EventRatingPanel({
  ...
  const handleChangePanelTypeClick = useCallback(
    (e: React.MouseEvent<HTMLButtonElement>) => {
      ...
      setType(dataset.tabName as string);
    },
    [setType],
  );
