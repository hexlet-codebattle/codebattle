...
function EventCalendarPanel({ tournaments }: EventCalendarPanelProps) {
  const handleOpenInstruction = useCallback((description?: string) => {
  ...
  return (
    <div className="d-flex flex-column w-100">
      ...
      <div className="d-flex flex-column w-100">
        <TournamentInfo
          id={tournaments[0]?.id}
          type={tournaments[0]?.state || TournamentStatusCodes.waitingParticipants}
        ...
        <TournamentInfo
          id={tournaments[1]?.id}
          type={tournaments[1]?.state || TournamentStatusCodes.waitingParticipants}
          name={i18next.t('Stage %{name}', { name: 1 })}
        ...
        <TournamentInfo
          id={tournaments[2]?.id}
          type={tournaments[2]?.state || TournamentStatusCodes.waitingParticipants}
          name={i18next.t('Stage %{name}', { name: 1 })}
          nameClassName="cb-text-transparent"
          data="01.06"
          time="12:00-12:30 (UTC+3)"
        ...
        <TournamentInfo
          id={tournaments[3]?.id}
          type={tournaments[3]?.state || TournamentStatusCodes.waitingParticipants}
        ...
        <TournamentInfo
          id={tournaments[4]?.id}
          type={tournaments[4]?.state || TournamentStatusCodes.waitingParticipants}
