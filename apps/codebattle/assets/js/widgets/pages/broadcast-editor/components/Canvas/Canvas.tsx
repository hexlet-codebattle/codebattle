...
import { type Block, type BlockCommonProps } from '../../types';
import './Canvas.css';
...
function Canvas({ initialBlocks = [], onBlocksChange = () => {}, readOnly = false }: CanvasProps) {
  ...
  const handleAddBlock = (id: string) => {
    if (readOnly) return;
    const hasBlock = (blockId: string) => blocks.some((b) => b.id === blockId);
