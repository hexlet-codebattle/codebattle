...
interface CheckResult {
  ...
  successCount?: number;
  success?: number;
  asserts_count?: number;
  assertsCount?: number;
  asserts?: Array<{ status?: string; output?: string }>;
  status?: string;
  output?: string;
  output_error?: string;
  outputError?: string;
...
interface TestProgress {
  ...
  output: string;
  outputError: string;
  failedAssert?: { status?: string; output?: string };
}
...
interface ChannelPayload {
  ...
  game?: { players?: StreamPlayer[] } & Record<string, unknown>;
  state?: string;
  game_state?: string;
  gameState?: string;
...
const getPlayerId = (player?: StreamPlayer | null) => player?.id;
const getPlayerName = (player?: StreamPlayer | null) => player?.name || 'Unknown';
const getPlayerLang = (player?: StreamPlayer | null) =>
  player?.editorLang || player?.editor_lang || 'js';
const getPlayerText = (player?: StreamPlayer | null) =>
  player?.editorText || player?.editor_text || '';

const normalizePlayer = (player: StreamPlayer, fallback: StreamPlayer = {}): StreamPlayer => {
...
const normalizePlayers = (
  players: StreamPlayer[] = [],
  prevPlayers: StreamPlayer[] = [],
): StreamPlayer[] => {
  const prevById = prevPlayers.reduce<Record<string, StreamPlayer>>(
    (acc, player) => ({ ...acc, [String(getPlayerId(player))]: player }),
    {},
  );

  return players.map((player) => {
    const id = getPlayerId(player);
    return normalizePlayer(player, prevById[String(id)]);
  });
};

const updatePlayerEditorData = (
  players: StreamPlayer[],
  userId: number | string,
  editorText: string,
  editorLang?: string,
...
const pickPlayers = (payload: ChannelPayload = {}): StreamPlayer[] | null => {
...
const stateFromPayload = (payload: ChannelPayload = {}) =>
  payload.state || payload.game_state || payload.gameState || null;

const getTypingIntensity = (typingMeta: TypingMeta | undefined, now: number) => {
...
const getOutcomeIds = (players: StreamPlayer[] = []) => {
...
const parseTestProgress = (payload: ChannelPayload = {}): TestProgress => {
  const checkResult: CheckResult = payload.check_result || payload.checkResult || {};
...
const testsFromResult = (player: StreamPlayer): TestProgress => {
...
const testsFromPlayers = (players: StreamPlayer[] = []): Record<string, TestProgress> =>
  players.reduce<Record<string, TestProgress>>((acc, player) => {
    const id = getPlayerId(player);
    if (id == null) {
      return acc;
    }

    const checkResult = (player?.check_result || player?.checkResult) as CheckResult | undefined;
...
interface EditorBorderStateArgs {
  isChecking: boolean;
  isWinner: boolean;
  isLoser: boolean;
...
// Monaco editor options — cast to keep string-literal unions (lineNumbers/wordWrap/…) happy
// without pulling monaco's option types into this stream-only view.
...
interface Preset {
  layouts?: Record<string, Layout | undefined>;
  fontSizes?: Record<string, number>;
  zOrder?: Record<string, number>;
  paneHeaders?: Record<string, boolean>;
}

const loadPreset = (): Preset | null => {
  try {
...
const computeDefaultLayouts = (stage: HTMLElement): Record<WidgetId, Layout> => {
...
function fontButtonStyle(accent: string): React.CSSProperties {
  return {
...
interface PaneProps {
  ...
  zIndex?: number;
  fontSize?: number;
  onIncreaseFont?: MouseHandler;
  onDecreaseFont?: MouseHandler;
  onDragStart?: MouseHandler;
  onResizeStart?: MouseHandler;
  onBringToFront?: MouseHandler;
  bodyStyle?: React.CSSProperties;
  children?: React.ReactNode;
  editMode?: boolean;
  showHeader?: boolean;
  onToggleHeader?: MouseHandler;
}
...
// Monaco's onMount gives (editor, monaco); neither is typed locally here.
type MonacoMountHandler = (editor: any, monaco: any) => void;
...
function EditorBody({
  ...
  const language = languages[getPlayerLang(player) as keyof typeof languages] || 'javascript';
...
function TaskBody({ description, fontSize, transparent = false }: TaskBodyProps) {
  return (
    <div
...
interface ExamplesBodyProps {
  examples: string;
  fontSize: number;
}

function ExamplesBody({ examples, fontSize }: ExamplesBodyProps) {
  return (
    <div
...
function TimerBody({ deadlineMs, gameState, kiosk = false }: TimerBodyProps) {
...
interface BattleState {
  ...
  typing: Record<string, TypingMeta>;
  tests: Record<string, TestProgress>;
  fx: FxState;
}

interface ThreejsGamePageProps {
...
function ThreejsGamePage({
  ...
  // Monaco editor + api instances have no local types in this stream-only view.
  const monacoApiRef = useRef<any>(null);
  const editorRefs = useRef<Record<string, any>>({});
  const remoteDecorationsRef = useRef<Record<string, { cursor: string[]; selection: string[] }>>(
    {},
  );

  const [isFullscreen, setIsFullscreen] = useState(false);
  const [editMode, setEditMode] = useState(false);
  const [paneHeaders, setPaneHeaders] = useState<Record<string, boolean>>(() => {
  ...
  const togglePaneHeader = (id: string) => setPaneHeaders((prev) => ({ ...prev, [id]: !prev[id] }));
  ...
  useEffect(() => {
    ...
    const isValidLayout = (l?: Layout): l is Layout =>
      !!l && typeof l.x === 'number' && typeof l.y === 'number' && l.width >= 60 && l.height >= 60;

    const initLayouts = () => {
      const defaults = computeDefaults();
      const preset = loadPreset();
      const merged: Record<string, Layout> = {};
      (Object.keys(defaults) as WidgetId[]).forEach((key) => {
        const presetLayout = preset?.layouts?.[key];
        merged[key] = isValidLayout(presetLayout) ? presetLayout : defaults[key];
      });
  ...
  const startInteraction = (id: string, mode: 'drag' | 'resize') => (e: React.MouseEvent) => {
  ...
  const adjustFont = (id: string, delta: number) =>
    setFontSizes((prev) => ({
      ...prev,
      [id]: clamp(
        prev[id] + delta,
  ...
  useEffect(() => {
    ...
    const refs: Array<{ event: string; ref: unknown }> = [];

    const appendEvent = (event: string, payload: ChannelPayload) => {
      const eventUserId = payload.user_id || payload.userId;

      const applyRemoteCursor = (userId: number | string | undefined, offset?: number) => {
        const editor = userId != null ? editorRefs.current[userId] : undefined;
        const monaco = monacoApiRef.current;
        if (userId == null || !editor || !monaco || typeof offset !== 'number') {
          return;
        }
    ...
    const addHandler = (event: string) => {
      const ref = channel.on(event, (payload: ChannelPayload) => appendEvent(event, payload || {}));
      refs.push({ event, ref });
    };
    ...
    channel.join().receive('ok', (resp: { game?: GamePayload }) => {
  ...
  useEffect(() => {
    ...
    // three.js meshes/materials are untyped (CDN import); keep particle handling loose.
    const runtime: { cleanup: (() => void) | null } = { cleanup: null };
    ...
    const init = async () => {
      ...
      const spawnBurst = (x: number, colorHex: string, count = 32) => {
  ...
  const getBorder = (
    player: StreamPlayer | null,
    theme: { frame: string; header: string },
  ): Border => {
    const playerId = getPlayerId(player);
    const checking = Boolean(playerId != null && battleState.checking[playerId]);
    const typingIntensity = getTypingIntensity(
      playerId != null ? battleState.typing[playerId] : undefined,
      now,
    );
  ...
  const testsFor = (player: StreamPlayer | null): TestProgress | undefined => {
    const id = getPlayerId(player);
    return id != null ? battleState.tests[id] : undefined;
  };
  ...
  const registerEditor = (userId: number | string | undefined) => (editor: any, monaco: any) => {
    if (userId != null) {
  ...
  if (kioskWidget) {
    if (!KIOSK_WIDGETS.has(kioskWidget)) {
      return <KioskWaiting />;
    }
