...
interface GameRoomTimerProps {
  timeoutSeconds: number | null;
  time: string | null;
}

function GameRoomTimer({ timeoutSeconds, time }: GameRoomTimerProps) {
...
interface GameOverTimerProps {
  timeoutSeconds: number | null;
  time: string | null;
  durationSec: number | null;
}

function GameOverTimer({ timeoutSeconds, time, durationSec }: GameOverTimerProps) {
  const [remaining] = useState<string | null>(() => {
    if (!timeoutSeconds) {
      return null;
    }
...
function TimerContainer() {
  ...
  if (isGameStored) {
    return gameStatuses[gameStateName as keyof typeof gameStatuses];
  }
