...
const decorateJoinedDate = (str: string) => moment.utc(str).format('LL');

const renderSortArrow = (attribute: string, sortParams: SortParams) => {
...
const renderUser = (page: number, pageSize: number, user: RatingUser, index: number) => (
  <tr key={user.id}>
...
const renderPagination = (
  { pageInfo: { pageNumber, pageSize, totalEntries } }: PageInfoData,
...
function UsersRating() {
  ...
  useEffect(() => {
    dispatch(getUsersRatingPage(filterParams, sortParams, page, Number(pageSize)));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [filterParams, sortParams, page, pageSize]);

  const triggerSort = (attribute: string) => {
  ...
  return (
    <div className="text-center cb-bg-panel cb-rounded p-3">
      ...
      <div>{renderPagination(usersRatingPage as unknown as PageInfoData, setPage)}</div>
    </div>
  );
}
