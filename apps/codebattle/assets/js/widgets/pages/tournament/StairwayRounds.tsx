...
interface StairwayRoundTask {
  roundId: number;
  status: string;
}

interface StairwayRoundsPlayer {
  id: number;
  tasks: StairwayRoundTask[];
  [key: string]: unknown;
}

interface StairwayRoundsProps {
  players: StairwayRoundsPlayer[];
  activePlayerId: number;
  activeRoundId: number;
  setActiveRoundId: (roundId: number) => void;
}
...
function StairwayRounds({
  ...
  const renderRoundTabs = currentUser!.tasks.map(({ roundId, status }: StairwayRoundTask) => {
