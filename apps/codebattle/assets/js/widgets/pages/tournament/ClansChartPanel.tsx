...
interface ClansChartPanelProps {
  type: string;
  state: string;
}

// chart.js data items ship no typed shape from the backend; keep pragmatic
interface ClanChartItem {
  clanId: number;
  clanName?: string;
  playerCount?: number;
  totalScore?: number;
...
function ClansChartPanel({ type, state }: ClansChartPanelProps) {
  const dispatch = useDispatch<AppDispatch>();

  const chartRef = useRef<HTMLDivElement>(null);
  const [items, setItems] = useState<ClanChartItem[]>([]);
