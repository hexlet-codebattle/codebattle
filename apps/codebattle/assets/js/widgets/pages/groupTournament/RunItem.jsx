  ...
  return (
    <div key={item.id} className="mb-2">
      ...
      >
        ...
        <div className={`d-flex flex-wrap align-items-center small mt-1 w-100 ${isActive ? "text-white-50" : "text-muted"}`}>
          {pending ? (
            i18n.t("Running…")
          ) : (
            <>
              <span className="font-weight-bold mr-3 text-nowrap text-white" style={{ opacity: 0.9 }}>
                {item.status === 'error' && i18n.t("Error")}
                {item.status === 'timeout' && i18n.t("Time Limit")}
                {item.status !== 'error' && item.status !== 'timeout' && i18n.t("Score %{score}", { score: item.score ?? 0 })}
              </span>
              {roundRun && (
                <span className="font-weight-bold ml-auto text-nowrap text-white" style={{ opacity: 0.75 }}>
