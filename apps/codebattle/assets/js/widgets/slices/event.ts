...
import initial, { type EventState } from './initial';
...
const fetchCommonLeaderboard = createAsyncThunk(
  ...
  async (params: LeaderboardParams, { getState }) => {
    ...
    const query = new URLSearchParams(
      decamelizeKeys(params, { separator: '_' }) as unknown as Record<string, string>,
    ).toString();
...
const eventSlice = createSlice({
  ...
  reducers: {
    initEvent: (_state, { payload }: PayloadAction<Record<string, unknown>>) => ({
      ...payload,
      loading: loadingStatuses.PENDING,
    }),
    updateEvent: (state, { payload }: PayloadAction<Partial<EventState>>) => ({
      ...state,
      ...payload,
    }),
  },
...
const { actions: sliceActions, reducer } = eventSlice;

const actions = {
  ...sliceActions,
  fetchCommonLeaderboard,
};
