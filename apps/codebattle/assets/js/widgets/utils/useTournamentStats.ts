...
const getActiveGameId = (gameStatus: any, gameId: any) => {
...
const useTournamentStats = ({ type }: { type: string }) => {
  ...
  const taskCount = (user?.taskIds as unknown[] | undefined)?.length || 1;
  const taskSolvedCount = user?.state === 'active' ? taskCount - 1 : taskCount;

  const activeGameId =
    type === 'tournament'
      ? // eslint-disable-next-line @typescript-eslint/no-explicit-any
        (Object.values(matches) as any[]).find(
          (match) => match.state === MatchStateCodes.playing && match.playerIds.includes(user?.id),
        )?.gameId
  ...
  return {
    ...
    taskSolvedCount,
    breakState,
    maxPlayerTasks: (taskIds as unknown[] | undefined)?.length,
    activeGameId,
  };
};
