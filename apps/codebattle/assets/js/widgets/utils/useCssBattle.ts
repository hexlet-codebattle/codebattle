...
const getIframeHtmlContent = (text: string) => `<div></div><style>${text}</style>`;

interface CssBattleEditor {
  text: string;
  userId: number;
}
// const defaultText = '\ndiv {\n\tbackground: #F3AC3C;\n\twidth: 50px;\n\theight: 50px\n}';
...
interface MatchStats {
  status: string;
  result: MatchResultEntry[];
  diffDataUrl?: string;
  targetCanvas?: HTMLCanvasElement;
}

const useIframes = () => {
  const leftSolutionIframe = useRef<HTMLIFrameElement>(null);
  const rightSolutionIframe = useRef<HTMLIFrameElement>(null);
...
const useCssBattleStats = (
  isIframesLoaded: boolean,
  leftSolutionIframe: React.RefObject<HTMLIFrameElement | null>,
  rightSolutionIframe: React.RefObject<HTMLIFrameElement | null>,
) => {
  ...
  const [leftDataUrl, setLeftDataUrl] = useState<string>();
  const [rightDataUrl, setRightDataUrl] = useState<string>();
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const rightEditor = useSelector((selectors.rightEditorSelector as any)()) as CssBattleEditor;
  ...
  useEffect(() => {
    if (leftImgRef.current) {
      leftImgRef.current.src = leftDataUrl as string;
    }
  }, [leftDataUrl]);

  useEffect(() => {
    if (rightImgRef.current) {
      rightImgRef.current.src = rightDataUrl as string;
    }
  }, [rightDataUrl]);
