...
// @ts-expect-error pre-existing unresolved module; this hook is currently
// unused and its `./builder` helper does not exist in the tree.
...
const isValidValueToSignature = (value: unknown, signature?: Signature): boolean => {
  if (!signature) {
    return false;
  }
...
interface UseValidationExampleParams {
  suggest?: Suggest;
  inputSignature: SignatureEntry[];
...
const useValidationExample = ({
  ...
  const [validArguments, reasonInvalidArguments] = useMemo(() => {
    try {
      ...
      const data = JSON.parse(suggest.arguments as string);
  ...
  const [validExpected, reasonInvalidExpected] = useMemo(() => {
    try {
      ...
      const data = JSON.parse(suggest.expected as string);
