...
interface UseEditorProps extends UseOptionProps {
  roomMode?: string;
  checkResult?: () => void;
  ...
  onTelemetryEvent?: (payload: Record<string, any>) => void;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  [key: string]: any;
}

// Monaco editor instance is typed loosely: no lightweight shared type is
...
const useOption = (
  ...
  const options = useMemo(
    () => ({
      ...
      tabSize: getLanguageTabSize(syntax as string),
      insertSpaces: shouldReplaceTabsWithSpaces(syntax as string),
...
const useEditor = (props: UseEditorProps) => {
  ...
  useResizeListener(editor, props as { locked?: boolean });

  // Prevent browser "Save Page" on Ctrl+S / Cmd+S
  const handleEnterCtrPlusS = useCallback((e: KeyboardEvent) => {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const handleEditorDidMount = (currentEditor: any, currentMonaco: any) => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    currentEditor.onDidChangeModelContent((event: any) => {
    ...
    if (!allowClipboard) {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      currentEditor.onContextMenu((e: any) => {
    ...
    if (!allowClipboard)
      domNode.addEventListener(
        'paste',
        (e: ClipboardEvent & Partial<ModifierKeyState>) => {
          emitTelemetry({
            type: 'paste_blocked',
