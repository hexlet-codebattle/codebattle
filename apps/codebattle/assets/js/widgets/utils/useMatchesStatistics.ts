...
type PlayerResults = Record<number, PlayerResult>;
 ...
 * @param matches - list of matches per round
 */
function useMatchesStatistics(
  ...
  return useMemo(() => {
    ...
    const playerAvgTests =
      matchesCount !== 0
        ? sum(finishedMatches.map((match) => match.playerResults[playerId]?.resultPercent || 0)) /
          matchesCount
        : 0;
    const opponentAvgTests =
      matchesCount !== 0
        ? sum(
            finishedMatches.map(
              (match) => match.playerResults[opponentId as number]?.resultPercent || 0,
            ),
          ) / matchesCount
        : 0;

    const playerAvgDuration =
      finishedMatches.filter((match) => match.winnerId === playerId).length !== 0
        ? sum(
            finishedMatches
              .filter((match) => match.winnerId === playerId)
              .map((match) => match?.durationSec || 0),
          ) / finishedMatches.filter((match) => match.winnerId === playerId).length
        : 0;
    const opponentAvgDuration =
      finishedMatches.filter((match) => match.winnerId === opponentId).length !== 0
        ? sum(
            finishedMatches
              .filter((match) => match.winnerId === opponentId)
              .map((match) => match?.durationSec || 0),
