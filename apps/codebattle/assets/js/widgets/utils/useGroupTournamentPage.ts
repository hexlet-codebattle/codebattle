...
const useGroupTournamentPage = (tournamentId?: number | string | null) => {
  ...
  const handleSelectRun = (id: number | string) => {
    setSelectedRunId(id);
    setActiveTab('run');
  };
  ...
  const handleSubmitSolution = (solution: string, lang: string) =>
    submitSolution(solution, lang)(dispatch);
