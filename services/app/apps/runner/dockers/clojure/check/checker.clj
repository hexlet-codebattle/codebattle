(try
  ...
  (catch Exception ex
    (do
     (println (cheshire.core/generate-string {:status "error" :result (.getMessage ex)}))
     (System/exit 0)
     )))
...
(defn assert-result
  ...
  (let [info (get-solution-result solution args)
    ...
    (try (assert (= expected result))
      (println (cheshire.core/generate-string {:status "success" :result result :expected expected :output output :arguments args :execution_time execution-time}))
      true
    (catch java.lang.AssertionError e
      (do
        (println (cheshire.core/generate-string {:status "failure" :result result :expected expected :output output :arguments args :execution_time execution-time}))
        false
        )))))
