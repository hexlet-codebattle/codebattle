...
function useRoundStatistics(
  ...
  const opponentAvgTests = matchesCount !== 0
    ? sum(finishedMatches.map(match => match.playerResults[opponentId]?.resultPercent || 0)) / matchesCount
    : 0;

  const playerAvgDuration = matchesCount !== 0
    ? sum(finishedMatches.map(match => match.playerResults[playerId]?.durationSec || 0)) / matchesCount
    : 0;
  const opponentAvgDuration = matchesCount !== 0
    ? sum(finishedMatches.map(match => match.playerResults[opponentId]?.durationSec || 0)) / matchesCount
    : 0;
