  ...
  if (!gradeStats || gradeStats.length === 0) {
    return (
      <div className="text-center text-muted py-3">
        No tournament data by grade available
      </div>
    );
  }
          ...
          {gradeStats.map((g) => (
            <tr key={g.grade}>
              <td>
                <span
                  className="fw-bold"
                  style={{ color: GRADE_COLORS[g.grade] || '#666' }}
                >
          ...
          {tournaments.map((t) => (
            <tr key={t.tournament_id}>
              ...
              <td className="text-center">
                <span
                  className="fw-bold"
                  style={{ color: GRADE_COLORS[t.grade] || '#666' }}
                >
              ...
              <td className="text-center">
                <span className="text-success">{t.wins_count}</span>
                /
                {t.games_count}
              </td>
            </tr>
...
function PlayerInsightsModal({
  show, onHide, player, allResults, season,
}) {
  ...
  useEffect(() => {
    if (show && player && season) {
      ...
      axios.get(`/api/v1/seasons/${season.id}/players/${player.user_id}/stats`)
        .then((response) => {
  ...
  const medianStats = useMemo(() => {
    ...
    const sortedTournaments = [...allResults].sort((a, b) => a.tournaments_count - b.tournaments_count);
    const sortedScore = [...allResults].sort((a, b) => a.total_score - b.total_score);

    const mid = Math.floor(allResults.length / 2);
    const getMedian = (arr) => (arr.length % 2 === 0
  ...
  const percentile = totalPlayers > 0
    ? Math.round(((totalPlayers - player.place) / totalPlayers) * 100)
    : 0;
  ...
  const playerWinRate = player.total_games_count > 0
          ...
          <div className="d-flex align-items-center justify-content-between">
            ...
            <div className="d-flex align-items-center">
              ...
              <div>
                ...
                <div className="text-muted small">
                  ...
                  {season?.year}
                  {player.clan_name && (
                    <span className="text-info ml-2">{player.clan_name}</span>
                  )}
                </div>
              </div>
            </div>
            ...
            <div className="d-flex mr-5">
              <div className="text-center px-4">
                <div className="fs-4 fw-bold text-warning">{player.total_points.toLocaleString()}</div>
            ...
            {activeTab === 'overview' && (
              <div className="row">
                ...
                <div className="col-md-5">
                  ...
                  {player.user_lang && (
                    <div className="mb-4 text-center">
                      ...
                      <LanguageIcon lang={player.user_lang} style={{ width: '80px', height: '80px' }} />
                ...
                <div className="col-md-7">
                  {/* Win Rate Chart */}
                  <WinRateChart
                    wins={player.total_wins_count}
                    total={player.total_games_count}
                  />
                      ...
                      <div className="card-body">
                        ...
                        <div className="row small">
                          <div className="col-6">
                            ...
                            <span className={cn('fw-bold', player.total_points >= medianStats.points ? 'text-success' : 'text-danger')}>
                          ...
                          <div className="col-6">
                            ...
                            <span className={cn('fw-bold', playerWinRate >= medianStats.winRate ? 'text-success' : 'text-danger')}>
                          ...
                          <div className="col-6">
                            ...
                            <span className={cn(
                              'fw-bold',
                              player.total_wins_count >= medianStats.wins ? 'text-success' : 'text-danger',
                            )}
                            >
                          ...
                          <div className="col-6">
                            ...
                            <span className={cn(
                              'fw-bold',
                              player.tournaments_count >= medianStats.tournaments ? 'text-success' : 'text-danger',
                            )}
                            >
                  ...
                  <div className="row mt-4">
                    ...
                    <div className="col-md-6 mb-4">
                      <div className="card cb-bg-panel border-0 h-100">
                        <div className="card-body">
                          <h6 className="text-muted small text-uppercase">Stats vs Median (Normalized)</h6>
                          <ResponsiveContainer width="100%" height={250}>
                            <RadarChart data={[
                              {
                                stat: 'Points',
                                player: Math.min((player.total_points / Math.max(medianStats.points, 1)) * 50, 100),
                                median: 50,
                              },
                              {
                                stat: 'Wins',
                                player: Math.min((player.total_wins_count / Math.max(medianStats.wins, 1)) * 50, 100),
                                median: 50,
                              },
                              {
                                stat: 'Win Rate',
                                player: Math.min((playerWinRate / Math.max(medianStats.winRate, 1)) * 50, 100),
                                median: 50,
                              },
                              {
                                stat: 'Score',
                                player: Math.min((player.total_score / Math.max(medianStats.score, 1)) * 50, 100),
                                median: 50,
                              },
                              {
                                stat: 'Tournaments',
                                player: Math.min((player.tournaments_count / Math.max(medianStats.tournaments, 1)) * 50, 100),
                                median: 50,
                              },
                            ]}
                            >
                              ...
                              <Radar name="You" dataKey="player" stroke="#0dcaf0" fill="#0dcaf0" fillOpacity={0.5} />
                              <Radar name="Median" dataKey="median" stroke="#6c757d" fill="#6c757d" fillOpacity={0.2} />
                              <Legend />
                            </RadarChart>
                          </ResponsiveContainer>
                        </div>
                      ...
                      <div className="card cb-bg-panel border-0 h-100">
                        <div className="card-body">
                          ...
                          <ResponsiveContainer width="100%" height={250}>
                            <BarChart
                              data={[
                                { name: 'Points', you: player.total_points, median: medianStats.points },
                                { name: 'Score', you: player.total_score, median: medianStats.score },
                                { name: 'Wins', you: player.total_wins_count, median: medianStats.wins },
                                { name: 'Games', you: player.total_games_count, median: medianStats.games },
                              ]}
                              layout="vertical"
                            >
                              <CartesianGrid strokeDasharray="3 3" stroke="#444" />
                      ...
                      <div className="card cb-bg-panel border-0 h-100">
                        <div className="card-body">
                          ...
                          <ResponsiveContainer width="100%" height={200}>
                            <BarChart
                              data={[
                                { name: 'Win Rate %', you: playerWinRate, median: Math.round(medianStats.winRate) },
                              ]}
                            >
                              <CartesianGrid strokeDasharray="3 3" stroke="#444" />
                    ...
                    <div className="col-md-6 mb-4">
                      <div className="card cb-bg-panel border-0 h-100">
                        <div className="card-body">
                          <h6 className="text-muted small text-uppercase">Your Ranking Percentile</h6>
                          <ResponsiveContainer width="100%" height={200}>
                            <PieChart>
                              <Pie
                                data={[
                ...
                {detailedStats?.grade_stats && (
                  <div className="row mt-2">
                    <div className="col-md-6">
                      <div className="card cb-bg-panel border-0">
                        <div className="card-body">
                          <h6 className="text-muted small text-uppercase">Points Distribution by Grade</h6>
                          <ResponsiveContainer width="100%" height={200}>
                            <BarChart data={ALL_GRADES.map((grade) => {
                              const gradeData = detailedStats.grade_stats.find((g) => g.grade === grade);
                              return {
                                name: formatGradeName(grade),
                    ...
                    <div className="col-md-6">
                      <div className="card cb-bg-panel border-0">
                        <div className="card-body">
                          ...
                          <ResponsiveContainer width="100%" height={200}>
                            <BarChart data={detailedStats.grade_stats.map((g) => ({
