...
function TeamMatches({ currentUserId, matches }) {
        ...
        <div className="col-12 mt-3 py-2 bg-white shadow-sm rounded">
          <div className="row mb-3">
            ...
            <div className="col-1 text-center">
              <span className="font-weight-light mb-0">{calcRoundResult(roundMatches).first}</span>
            </div>
            <div className="col-1 text-center">
              <span className="font-weight-light mb-0">{calcRoundResult(roundMatches).second}</span>
            </div>
            $
            {roundMatches.map((match) => {
              ...
              return (
                <div className={`row align-items-center py-2 ${bgClass}`}>
                  <div className="col-6">
                    <UserInfo hideOnlineIndicator user={player1} />
                  </div>
                  <div className="col-4">
                    <UserInfo hideOnlineIndicator user={player2} />
                  </div>
                  <div className="col-2 text-right">
                    <a className="btn btn-success" href={`/games/${match.gameId}`}>
                      {linkName}
                    </a>
                  </div>
                </div>
