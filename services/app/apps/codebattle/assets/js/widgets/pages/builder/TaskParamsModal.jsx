...
const getTitle = (action, taskParamsState) => {
  switch (action) {
    ...
    case "save": {
      const title =
        taskParamsState === taskStateCodes.blank ? "Confirm task creation" : "Confirm task changes";
      return title;
    }
    case "none": {
      return "";
...
const TaskParamsModal = NiceModal.create(
  ({ taskService, mode: defaultMode = "none", action = "none" }) => {
    ...
    const taskParamsJSON = JSON.stringify(omit(taskParams, ["creatorId", "id", "state"]), null, 2);

    const title = useMemo(() => getTitle(action, taskParams.state), [action, taskParams.state]);
    const btnTitle = useMemo(() => getBtnTitle(action, state), [action, state]);

    const modal = useModal(ModalCodes.taskParamsModal);

    const getContentClassnameByMode = useCallback(
      (currentMode) =>
        cn("p-3", {
    ...
    const handleChangeMode = useCallback(
      (event) => {
        const nextMode = event.target.checked ? modalModes.showJSON : modalModes.preview;
      ...
      try {
        switch (action) {
          ...
          case "none":
            break;
          default:
            throw new Error(`Unexpected task builder action type: ${action}`);
        }
      } catch (err) {
        setError(err);
      }
    ...
    useEffect(() => {
      if (
        modal.visible &&
        !isEmpty(value) &&
        [modalActions.upload, modalActions.edit].includes(action)
      ) {
