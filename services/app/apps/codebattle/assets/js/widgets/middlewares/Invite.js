...
export const initInvites = (currentUser) => (dispatch) => {
  const onJoinSuccess = () => {
    ...
    channel.on('invites:expired', (data) => {
      const message = getSystemMessage({
        text: `Invite has been expired by timeout (${getCreatorName(data)} vs ${getRecipientName(
          data,
        )})`,
        status: 'failure',
      });
...
export const createInvite = (params) => (dispatch) =>
  channel.push('invites:create', params).receive('ok', (data) => {
    const message = getSystemMessage({
      text: `You invite ${params.recipient_name} to battle. Wait for his reply`,
    });
    dispatch(actions.newChatMessage(message));
...
export const acceptInvite = (id) => (dispatch) =>
  channel.push('invites:accept', { id }).receive('ok', (data) => {
    setTimeout(() => {
      window.location.href = `/games/${data.invite.game_id}`;
    }, 250);
...
export const declineInvite = (id, opponentName) => (dispatch) =>
  channel.push('invites:cancel', { id }).receive('ok', (data) => {
    const message = getSystemMessage({
