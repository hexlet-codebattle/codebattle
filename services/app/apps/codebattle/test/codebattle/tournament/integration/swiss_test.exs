defmodule Codebattle.Tournament.Integration.SwissTest do
  ...
  describe "Full tournament" do
    test "works",
      ...
      assert %Player{score: 100, wins_count: 1} = tournament |> get_player(user_id1)

      Tournament.Server.finish_round_after(tournament.id, tournament.current_round_position, 0)
      :timer.sleep(100)

      assert %Player{score: 101, wins_count: 1} = tournament |> get_player(user_id1)
      tournament = Tournament.Server.get_tournament(tournament.id)
      ...
      assert %Player{score: 51, wins_count: 0} = tournament |> get_player(opponent_id)
      assert %Player{score: 201, wins_count: 2} = tournament |> get_player(user_id1)
      ...
      assert %Player{score: 201, wins_count: 0} = tournament |> get_player(opponent_id)
      assert %Player{score: 501, wins_count: 5} = tournament |> get_player(user_id1)
