defmodule Codebattle.CodeCheck.OutputParserTest do
  ...
  @token %{lang_meta: Codebattle.Languages.meta("haskell"), seed: "123", raw_docker_output: ""}

  test "parses output with errors" do
    assert OutputParser.call(%{
             @token
             | raw_docker_output: ~s({"status": "error", "result": "sdf"})
           }) ==
             %Result{
               asserts_count: 0,
               success_count: 0,
  ...
  test "parses output with success" do
    assert OutputParser.call(%{
             @token
             | raw_docker_output: ~s({"status": "ok", "result": "__seed:123__"})
           }) ==
             %Result{
               asserts_count: 0,
               success_count: 0,
  ...
  test "parses output with failures" do
    assert OutputParser.call(%{
             @token
             | raw_docker_output: ~s({"status": "failure", "result": "0", "arguments": [0]}
