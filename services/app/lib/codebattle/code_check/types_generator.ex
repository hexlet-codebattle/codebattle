defmodule Codebattle.CodeCheck.TypesGenerator do
  @moduledoc ~S"""
      iex> Codebattle.CodeCheck.TypesGenerator.get_import(
      ...>    %{
      ...>      input_signature: [
      ...>        %{argument_name: "obj1", type: %{name: "hash", nested: %{name: "string"}}},
      ...>        %{argument_name: "arr1", type: %{name: "array", nested: %{name: "integer"}}},
      ...>        %{argument_name: "value", type: %{name: "integer"}},
      ...>        %{argument_name: "obj2", type: %{name: "hash", nested: %{name: "integer"}}},
      ...>      ],
      ...>      output_signature: %{type: %{name: "hash", nested: %{name: "float"}}}
      ...>    },
      ...>    Codebattle.Languages.meta("ts")
      ...> )
  ...
  def get_interface_name(%{argument_name: name}, "ts") do
    name
    |> case_words()
  ...
  def get_type(_signature, %{solution_version: :untyped}), do: ""
  ...
  def get_type(
    ...
    if is_hash?(signature) do
      get_interface_name(signature, meta.slug)
    else
  ...
  def get_type(%{type: %{name: name, nested: nested}}, meta) do
    type = Map.get(meta.types, name)
    EEx.eval_string(type, inner_type: get_type(%{type: nested}, meta))
  end

  def get_type(%{type: %{name: name}}, meta), do: Map.get(meta.types, name)
