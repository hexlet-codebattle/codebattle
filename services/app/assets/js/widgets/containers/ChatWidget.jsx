...
const ChatWidget = props => {
  ...
  const [isEmojiTooltipVisible, setIsEmojiTooltipVisible] = useState(false);
  useEffect(() => {
    const { dispatch, isStoredGame } = props;
    if (!isStoredGame) {
      dispatch(fetchState());
    }
  }, []);
  ...
  const handleChange = ({ target: { value } }) => {
    const EmojiTooltipVisible = /.*:[a-zA-Z]{1,}([^ ])+$/.test(value);
    setIsEmojiTooltipVisible(EmojiTooltipVisible);
    setMessage(value);
  };
  ...
  const hideEmojiTooltip = () => setIsEmojiTooltipVisible(false);
  ...
  const { messages, users, isStoredGame } = props;
