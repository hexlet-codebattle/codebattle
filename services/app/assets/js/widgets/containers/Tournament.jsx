...
const TournamentHeader = ({ state, startsAt }) => {
  ...
  const titles = {
    [TournamentStates.active]: (time) =>
      time > 0 ? (
        <span>
          The next round will start in&nbsp;
          {minutesToStart}:{secondsToStart}, or after all matches are over
        </span>
      ) : (
        <span>The tournament will start soon</span>
      ),
    [TournamentStates.waitingParticipants]: (time) =>
      time > 0 ? (
        <span>
          The tournament will start in&nbsp;
          {minutesToStart}:{secondsToStart}
        </span>
      ) : (
        <span>The tournament will start soon</span>
      ),
...
const IndividualMatches = memo(({ state, matches, playersCount }) => {
  ...
  const roundsRange = _.range(roundsCount);
  console.log(matches, playersCount, roundsRange);
      ...
      <div className="col-9 bg-white shadow-sm py-4">
        <div className="d-flex justify-content-around">
          {roundsRange.map(round => {
            const tournamentStage = playersCount / (2 ** round);
